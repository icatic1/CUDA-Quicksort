...
void test_thrustSort(uint* data, uint N, double* timer)
{

    //Type* h_data=(Type )malloc(N * sizeof(Type));
    uint* d_data;

    checkCudaErrors(cudaMalloc((void**)&d_data, (N) * sizeof(uint)));
    checkCudaErrors(cudaMemcpy(d_data, data, N * sizeof(uint), cudaMemcpyHostToDevice));
    ...
    thrust::device_ptr<uint> array(d_data);
